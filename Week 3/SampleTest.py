...
def test_case():
    ...
    try:
        ...
        dictionary = ans[0]
        flag = (0.244 <= dictionary['outlook'] <= 0.248) and (
                0.0292 <= dictionary['temp'] <= 0.0296) and (
                       0.150 <= dictionary['humidity'] <= 0.154) and (
                       0.046 <= dictionary['windy'] <= 0.05) and (ans[1] == 'outlook')
        if flag:
