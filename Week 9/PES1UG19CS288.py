...
class KMeansClustering:
    ...
    def e_step(self, data):
        ...
        """
        # Find distance from the 'M' data points to the 'self.n_clusters' centroids

        # Making a M X self.n_clusters matrix
        distances = np.zeros((data.shape[0], self.n_cluster))
        assignment = []

        for i in range(len(data)):
            for j in range(len(self.centroids)):
                sqd = np.sum(np.subtract(self.centroids[j], data[i]) ** 2)
                distances[i][j] = sqd

        for i in range(len(data[0])):
            assignment.append(np.argmin(distances[i]))
        return None
