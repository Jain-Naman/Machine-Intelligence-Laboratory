...
class AdaBoost:
    ...
    def stump_error(self, y, y_pred, sample_weights):
        ...
        return np.dot(error.transpose(), sample_weights) / np.sum(sample_weights)
    ...
    def compute_alpha(self, error):
        ...
        """
        eps = 1e-9
        alpha = (1 / 2) * np.log((1 - error + eps) / (error + eps))
        return alpha
    ...
    def update_weights(self, y, y_pred, sample_weights, alpha):
        ...
        wrong = y - y_pred
        wrong[wrong != 0] = 1
        correct = np.where((wrong == 0) | (wrong == 1), wrong ^ 1, wrong)

        correct_weights = np.multiply(correct, sample_weights) * (np.e ** (-alpha))
        wrong_weights = np.multiply(wrong, sample_weights) * (np.e ** alpha)

        new_weights = np.add(correct_weights, wrong_weights)
    ...
    def predict(self, X):
        ...
        prediction = np.array([self.stumps[i].predict(X) for i in range(self.n_stumps)]).transpose()
        alphas = np.array(self.alphas)
        alphas = np.tile(alphas, (prediction.shape[0], 1))
        # result = (np.sum(np.multiply(alphas, prediction), axis=1))
        result = []
        for i in prediction:
            unique, counts = np.unique(i, return_counts=True)
            freq = dict(zip(unique, counts))
            vote = max(freq, key=freq.get)
            priority = alphas[0][1]
            for index, k in enumerate(freq):
                if freq[k] == vote:
                    if alphas[0][index] > priority:
                        vote = k
            result.append(vote)
